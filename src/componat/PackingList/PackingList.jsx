        ...
        <ul className="w-[80%] md:w-[60%] lg:w-[50%] xl:w-[40%] mx-auto   shadow-xl">
          ...
          {filteredItems.length === 0 ? (
            ...
            filteredItems.map((task) => (
              <li
                key={task.id}
                className={`shadow-lg ${isDarkMode ? "bg-[#25273C] text-[hsl(0,0%,73%)]" : "bg-[hsl(0,0%,98%)] text-[#25273C]"} border-b ${isDarkMode ? "border-[hsl(233,14%,35%)]" : "border-[hsl(233,11%,84%)1]"} group p-5 w-[100%] flex items-center ${
                  task.package ? 'line-through text-gray-500' : ''
                }`}
              >
                <button
                  onClick={() => toggleComplete(task.id)}
                  className={`me-3 h-6 w-6 flex items-center justify-center rounded-full border-2 transition-all ${
                    task.package
